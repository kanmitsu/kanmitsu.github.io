...
async function decrypt(buffer, password) {
  ...
  // tag をデータの最後に結合する必要がある場合がある (Node.js の GCM タグの扱いと Web Crypto の違い)
  // Web Crypto API の AES-GCM expect tag at the end of the ciphertext
...
self.addEventListener('fetch', (event) => {
  ...
  let relativePath = url.pathname.slice(1); // 先頭の / を取る
  
  // ルートパスや index.html へのアクセスを処理
  ...
  // 資産が見つからない場合、または Service Worker がまだ準備できていない場合は通常のフェッチ
  // ただし、暗号化されている場合は元のファイルは存在しないはず
